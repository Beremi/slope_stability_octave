function [pw, grad_p, mater_sat]=seepage_problem_2D...
    (coord,elem,Q_w,pw_D,grho,conduct0,HatP,DHatP1,DHatP2,WF,linear_system_solver)

if nargin < 11 || isempty(linear_system_solver)
  ...
  eps=SEEPAGE.penalty_parameters_2D(coord,elem);

  % values of the penalization parameter at integration points
  eps_int=kron(eps,ones(1,n_q));

  % assembling of auxilliary matrices
  [Bw,C,weight]=SEEPAGE.auxiliary_matrices_2D(elem,coord,HatP,DHatP1,DHatP2,WF);  
 
  % assembling of the constitutive matrix D, size(D)=(2*n_int, 2*n_int)
  wc=weight.*conduct0; 
  vD=[1;0;0;1]*wc; % size(vD)=(4, n_int) 
  AUX=reshape(1:2*n_int,2,n_int);
  iD=repmat(AUX,2,1); 
  jD=kron(AUX,ones(2,1));
  D=sparse(iD,jD,vD);
