/*
 * constitutive_problem_2D_S_mex.c
 *
 * Mex wrapper for stress-only evaluation of the 2D Mohr-Coulomb
 * constitutive model. OpenMP-parallel over integration points.
 *
 * Usage:
 *   S = constitutive_problem_2D_S_mex(E, c_bar, sin_phi, shear, bulk, lame)
 *
 *   E        - 3 x n_int
 *   c_bar    - 1 x n_int
 *   sin_phi  - 1 x n_int
...
{
    if (nrhs != 6)
        mexErrMsgTxt("constitutive_problem_2D_S_mex: "
                     "6 inputs required (E, c_bar, sin_phi, shear, bulk, lame)");
    if (nlhs > 1)
        mexErrMsgTxt("constitutive_problem_2D_S_mex: 1 output (S)");
    ...
    if (mxGetM(prhs[0]) != 3)
        mexErrMsgTxt("constitutive_problem_2D_S_mex: E must have 3 rows.");
